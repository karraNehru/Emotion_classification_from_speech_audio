...
def load_model(model_path, device):
    model = models.efficientnet_b3(weights=None)  # EfficientNet-B3
...
def preprocess_audio(audio_file):
    ...
    fig = plt.figure(figsize=(3, 3), dpi=100)  # matches 300x300
...
if uploaded_file:
    with st.spinner('Analyzing emotion...'):
        ...
        model = load_model("emotion_b3_model.pth", device)  # <- updated model file

        image = preprocess_audio(uploaded_file)
        transform = transforms.Compose([
            transforms.Resize((300, 300)),  # updated input size
            transforms.ToTensor(),
